...
async def set_up_metadata():
    ...
    if not redis_cache.exists("CDS_META"):
        cds_meta = await get_cds_meta()
        redis_cache.set("CDS_META", pickle.dumps(cds_meta))
    if not redis_cache.exists("FDW_WRDS_TABLES"):
        async with sql_session_scope() as session:
            # get imported foreign tables
            res = await session.execute("select table_name from information_schema.tables where table_type='FOREIGN';")
            res = parse_sql_results(res)
            res = [d['table_name'] for d in res]
            redis_cache.set("FDW_WRDS_TABLES", pickle.dumps(res))
...
def get_top_gainers_api(time_range):
    try:
        ...
        return ResultResponse(status_code=CONSTS.HTTP_500_INTERNAL_SERVER_ERROR,
                              message=f"An exception occurred {str(e)}", debug=traceback.format_exc())
    return ResultResponse(status_code=CONSTS.HTTP_200_OK, content=result, message='Succeeded!')
...
async def get_cds_data(requestBody: CDSData):
    try:
        ...
        currency_query = f"""AND "CURRENCY" in ({', '.join(f"'{w}'" for w in requestBody['CURRENCY'])})""" if \
        requestBody[
            'CURRENCY'] and len(
        ...
        query = "SELECT * from cds " + where_clause + limit
        async with sql_session_scope() as session:
...
async def get_cds_meta():
    CDS_COLUMNS = ["REGION", "INDUSTRY", "OBLIGATION_ASSETRANK", "CREDIT_EVENTS"]
    result = {}
    async with sql_session_scope() as session:
        for col in CDS_COLUMNS:
            col_val = await session.execute(f"""SELECT DISTINCT "{col}" from cds""")
            col_val = parse_sql_results(col_val)
            col_val = [d[col] for d in col_val]
            result[col] = col_val
    return result
...
async def cds_get_unique_val(param: str):
    if redis_cache.exists("CDS_META"):
        return ResultResponse(status_code=CONSTS.HTTP_200_OK, content=pickle.loads(redis_cache.get("CDS_META"))[param])
    async with sql_session_scope() as session:
        result = await session.execute(f"""SELECT DISTINCT "{param}" from cds""")
        result = parse_sql_results(result)
...
async def fetch_available_tables():
    ...
    return ResultResponse(content=[{'value': i, 'label': i} for i in tables],
...
@router.get("/data_warehouse/fetch_available_fermi_tables")
@router_error_handler
async def fetch_available_tables():
    return ResultResponse(content=[{'value': i, 'label': i} for i in AVAIL_FERMI_DATA_TABLES],
                          status_code=CONSTS.HTTP_200_OK, message=f"Successfully queried available tables",
                          date_done=str(datetime.now(CONSTS.TIME_ZONE).isoformat()))
...
async def fetch_available_identifiers():
    ...
    return ResultResponse(content=avail_asset_identifiers, status_code=CONSTS.HTTP_200_OK,
...
async def get_table_cols(request: dict):
    ...
    return ResultResponse(content=result_dict, status_code=CONSTS.HTTP_200_OK,
...
async def get_integrated_data(request: dict):
    ...
    cols += ',' + f"""    TO_CHAR({table_names[0]}.{date_identifier}, 'YYYY/MM/DD') {date_identifier}""" \
        if date_identifier not in cols else ""
    ...
    result = await wrds_api.raw_sql(
        f"""SELECT {cols} FROM {from_tables} WHERE {table_names[0]}.{date_identifier} >= '{start_date}'
    ...
    return ResultResponse(content=result, status_code=CONSTS.HTTP_200_OK, message=f"Successfully queried data",
                          date_done=str(datetime.now(CONSTS.TIME_ZONE).isoformat()))


@router.post("/data_warehouse/run_select_query")
@router_error_handler
async def run_select_query(request: dict):
    async with sql_session_scope() as session:
        result = await session.execute(request['content'])
        result = parse_sql_results(result)
        return ResultResponse(content=result, status_code=CONSTS.HTTP_200_OK, message=f"Successfully queried data",
                              date_done=str(datetime.now(CONSTS.TIME_ZONE).isoformat()))
