...
async def login_redirect(auth_provider: str):
    ...
    async with exception_handling():
        ...
        response.set_cookie(key="state", value=f"Bearer {state_csrf_token}", **config.RESPONSE_COOKIE)
...
async def login(response: JSONResponse, internal_user: InternalUser = Depends(auth_token_scheme)) -> JSONResponse:
    ...
    async with exception_handling():
        ...
        response.set_cookie(key="access_token", value=f"Bearer {access_token}", **config.response_cookie)
