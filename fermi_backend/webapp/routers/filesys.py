...
async def search_file_by_name(name: str):
    async with sql_session_scope() as session:
        results = session.execute("SELECT * FROM fermi_db.filesys_idx WHERE name=:name;", {"name": name})
        results = parse_sql_results(results, orient='records')
        file_info = results[0]

    return ResultResponse(result=file_info, status_code=CONSTS.HTTP_200_OK, message=f"Successfully queried data",
