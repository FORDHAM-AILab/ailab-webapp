...
def router_error_handler(func):
    ...
    async def inner(*args, **kwargs):
        try:
            ...
            logger.exception(f"Failed to connect to the database: {repr(e)}. Detail: {traceback.format_exc()}")
            return ResultResponse(status_code=CONSTS.HTTP_600_DATABASE_CONNECTION_FAILED, message=f"Failed to connect to the database: {repr(e)}",
                                  debug=f"{str(e)}:\n{traceback.format_exc()}", date_done=datetime.now(CONSTS.TIME_ZONE).isoformat())
            ...
            return ResultResponse(status_code=CONSTS.HTTP_401_UNAUTHORIZED,
                                  ...
                                  debug=f"{str(e)}:\n{traceback.format_exc()}",
                                  date_done=datetime.now(CONSTS.TIME_ZONE).isoformat())
        except Exception as e:
            logger.error(f"An error has occurred. Please try again: {e}. Detail: {str(e)}:\n{traceback.format_exc()}")
