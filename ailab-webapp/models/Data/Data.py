...
class FinanceData:
    def __init__(self, approach='Yahoo'):
        ...
        # ----Input-----
        # approach: specify the df source, in current version, the Yahoo is the only available one
        # ----output----
    ...
    def columnNameMapping(columns):
        # Mapping the column names from different df source to unitized names
        # ----Input-----
    ...
    def getPrice(self, ticker, startDate, endDate, dateAscending=True):
        ...
        if (self.approach == 'Yahoo'):
            try:
                table = YahooFinancials(ticker)
                yahooPrices = \
                table.get_historical_price_data(startDate.isoformat(), endDate.isoformat(), 'daily')[ticker]['prices']
    ...
    def getPriceTable(self, tickerList, startDate=None, endDate=None, localCheck=None, dateAscending=True,
                      update=False):
        ...
        # endDate: the end date of price series, the format is 'YYYY-MM-DD'
        # dateAscending: whether rank the price series by date ascending, the default value is true
        # localCheck: loading local csv file, check the existing df see whether we need to retrieve df from Yahoo. The local file should contain date as index.
    ...
    def getDetailPriceInfo(self, ticker, startDate, endDate, columns=['close', 'date'], dateAscending=True,
                           frequency='D'):
    ...
    def dataMatching(self, df1, df2):
        ...
        df = pd.merge(df1, df2.iloc[:, :1], how='right', left_index=True, right_index=True).iloc[:, :-1].fillna(
            method='ffill')
        return df
