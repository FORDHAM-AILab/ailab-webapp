...
from ..webapp_models.generic_models import ResultResponse, CDSData, ESGData
import numpy as np
import pandas as pd
...
@router.post("/data_warehouse/get_esg_data")
def get_esg_data(requestBody: ESGData):
    mysql_engine = create_engine(config.MYSQL_CONNECTION_URL)
    try:
        requestBody = requestBody.dict()
        region_query = f"""REGION in ({', '.join(f'"{w}"' for w in requestBody['REGION'])}) """ if requestBody[
        ...
        where_clause = 'WHERE ' + conditions if conditions != '' else ''
        limit = f'limit {requestBody["limit"]}' if requestBody["limit"] else ""
        query = "SELECT * from fermi_db.esg " + where_clause + limit
        df = pd.read_sql(query, mysql_engine)
        df.replace({np.nan: None}, inplace=True)
        result = df.to_json(orient="records")
...
@router.get("/data_warehouse/esg_get_unique_val/{param}")
def esg_get_unique_val(param: str):
    mysql_engine = create_engine(config.MYSQL_CONNECTION_URL)

    try:

        query = f"SELECT DISTINCT {param} from fermi_db.esg"
        df = pd.read_sql(query, mysql_engine)
        result = list(df[param])
